...
-- ═══════════════════════════════════════════════════════
-- جدول جديد لتخزين بيانات الاختبارات (Quizzes)
-- ═══════════════════════════════════════════════════════
CREATE TABLE quizzes (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    quiz_name TEXT NOT NULL,
    file_id TEXT NOT NULL, -- معرف ملف الاختبار (Markdown) في تيليجرام
    source_file_name TEXT, -- اسم ملف المصدر الذي تم إنشاء الاختبار منه
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- إنشاء فهرس لسرعة البحث
CREATE INDEX idx_quizzes_source_file_name ON quizzes (source_file_name);

-- تمكين Row Level Security (RLS)
ALTER TABLE quizzes ENABLE ROW LEVEL SECURITY;

-- سياسات RLS (افتراضياً، يمكن للمستخدمين المصادق عليهم فقط القراءة والكتابة)
CREATE POLICY "Allow authenticated access to quizzes"
ON quizzes FOR ALL
TO authenticated
USING (true)
WITH CHECK (true);
